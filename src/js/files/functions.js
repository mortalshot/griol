...
export let lockOwner = null;

export function setLockOwner(owner) {
	lockOwner = owner;
}
...
export let bodyLock = (delay = 500, owner = null) => {
	if (bodyLockStatus) {
		...
		lockOwner = owner || 'manual'; // Запоминаем, кто залочил
...
export let bodyUnlock = (delay = 500, owner = null) => {
	// Только если запрашивает текущий владелец или явно сброс
...
export function setCloseSearchCallback(cb) {
	closeSearchCallback = cb;
}

export function callCloseSearchIfNeeded() {
	if (typeof closeSearchCallback === 'function') {
		closeSearchCallback();
	}
}

export function menuInit(onToggle = () => { }) {
  if (document.querySelector(".icon-menu button")) {
