...
(function (global, factory) {
  ...
  const hcSticky = function (elem, userSettings) {
    ...
    // --- Helpers shim (adds hcSticky.Helpers if missing) ---
    if (!hcSticky.Helpers) {
      hcSticky.Helpers = {
        getStyle(el, prop) {
        ...
        hasClass(el, cn) { return el.classList ? el.classList.contains(cn) : new RegExp('(^|\\b)' + cn.split(' ').join('|') + '(\\b|$)').test(el.className); },
        offset(el) {
        ...
        position(el) { return { top: el.offsetTop, left: el.offsetLeft }; },
        getElement(sel) { if (!sel) return null; if (typeof sel === 'string') return document.querySelector(sel); if (sel && sel.nodeType) return sel; return null; },
    ...
    const runSticky = () => {
      ...
      // Special range: viewport slightly taller than sticky but <= sticky + header
      ...
      if (offset_top > top_limit) {
        ...
        if (bottom_limit + options_top + (largerOrSpecial ? options_bottom : 0) - (STICKY_OPTIONS.followScroll && largerOrSpecial ? 0 : options_top) <= offset_top + sticky_height - stick_top - ((sticky_height - stick_top > window_height - (top_limit - stick_top) && STICKY_OPTIONS.followScroll) ? (((bottom_distance = sticky_height - window_height - stick_top) > 0) ? bottom_distance : 0) : 0)) { // bottom reached end
        ...
        else if (largerOrSpecial && STICKY_OPTIONS.followScroll) { // sticky is bigger than container and follows scroll
          if (scroll_dir === 'down') { // scroll down
            ...
            // If we were fixed to bottom and user starts scrolling up, switch to absolute so upper content can appear
            if (Sticky.position === 'fixed' && (elem.style.bottom !== 'auto')) {
              Sticky.release({
                position: 'absolute',
